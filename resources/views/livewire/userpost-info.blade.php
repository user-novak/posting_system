<div>
    {{-- @foreach ($images as $image)
        @if ($post->user->id == $image->imageable_id && $image->imageable_type == 'use App\Models\Profile')
            
            {{$profilebool}}
            <div class=" bg-white flex flex-wrap px-5 gap-5 my-4">
                <div class="py-5 px-5">
                    <figure>
                        <img class="object-center object-cover rounded-full h-48 w-48" src={{ asset($image->url) }}
                            alt="">
                    </figure>
                </div>
